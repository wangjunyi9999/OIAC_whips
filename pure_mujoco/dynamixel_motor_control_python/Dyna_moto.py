...
from Dynamixel_sdk.group_sync_read import *

from Dynamixel_sdk.group_sync_write import *
...
Z = 0.1
UNITCUR = 2.69 * 0.001
RPM = 0.229*2*pi/60.0# 0.0239808239224021

BAUDRATE = 3000000#4000000#57600#
class Dyna_moto():
	def __init__(self, baudrate = BAUDRATE, devicename = '/dev/ttyUSB0', protocolversion = 2.0, con_mode = POS_CONTROL, ro = False):
		...
		self.con_mode = zeros(self.moto_num, dtype = int)
		self.MinCur = zeros(self.moto_num)
		self.MaxCur = zeros(self.moto_num)
		self.MaxPos = zeros(self.moto_num)
		self.MinPos = zeros(self.moto_num)
		self.pos2rad = zeros(self.moto_num)

		self.MaxVel = zeros(self.moto_num)
		self.MinVel = zeros(self.moto_num)
		self.velrpm = RPM

		self.now_pos = zeros(self.moto_num)
		self.now_vel = zeros(self.moto_num)
		self.now_cur = zeros(self.moto_num)
		self.now_tor = zeros(self.moto_num)

		self.only_sense = ro

		self.en_tor = ones(self.moto_num, dtype = bool)

		'''
		if(self.only_sense):
			self.en_tor = zeros(self.moto_num, dtype = bool)
		else:
			self.en_tor = ones(self.moto_num, dtype = bool)
		'''
		...
		self.paras = zeros((5, self.moto_num))

		self.con_cur = zeros(self.moto_num, dtype = int)
		self.con_pos = zeros(self.moto_num, dtype = int)
		self.con_vel = zeros(self.moto_num, dtype = int)

		self.con_cur_n = zeros(self.moto_num)
		...
		self.init_var = zeros(self.moto_num)

		self.init_var = copy(self.now_pos)

		for i in range(self.moto_num):
			print("The initial position: %f" % self.init_var[i])

		self.con_pos = copy(self.now_pos)
		...
		#control parameters
		self.min_che_ang = pi*0.5
		self.man_che_ang = pi*1.5
	...
	def pos2ang(self, posi, maxposi, minposi):
		return(2*pi*(posi-minposi)/(maxposi-minposi))
	def ang2pos(self, angl, maxposi, minposi):
		return (minposi+(maxposi-minposi)*angl/(2*pi))
	def get_motor_ids(self):
		...
		for i in range(self.ping_num):
			...
			#print("test com",dxl_comm_result)
			if dxl_comm_result != COMM_SUCCESS:
				n=0
			elif dxl_error != 0:
				n=0
