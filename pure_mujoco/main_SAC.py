            ...
            if t<= args.start_timesteps:
                ...
                if args.target_type=='moving':
                    ...
                    if done:
                        ...
                        np.savetxt(f"./tmp_sac_new/{hit_tar}+{file_name}+{args.is_oiac}+{args.seed}+{t}+{train_pos}",action)                    
                        evaluation_hit.append(episode_timesteps) 
                        np.save(f"./results/{file_name}_{args.is_oiac}_{args.seed}_{args.target_type}_hits", evaluation_hit)   
                        episode_num += 1
                        hit_tar+=1
                    ...
                    if done:
                        ...
                        np.savetxt(f"./tmp_sac_new/{file_name}+{args.is_oiac}+{args.seed}+{t}+{train_pos}",action)                    
                        evaluation_hit.append(episode_timesteps) 
                        np.save(f"./results/{file_name}_{args.is_oiac}_{args.seed}_{args.target_type}_hits", evaluation_hit)
                        break
                    else:
