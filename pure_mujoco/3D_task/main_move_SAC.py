...
if __name__=="__main__":
    ...
    #train_pos=np.array([2,0, 0.0000])#T1
            ...
            if t<= args.start_timesteps:
                ...
                if t% args.save_freq==0:               
                    if args.save_freq>1:
                        ...
                        np.save(f"results/{file_name}_{stamp}_{args.is_oiac}_{args.seed}_{args.target_type}_{train_pos}_training_random", evaluation_random)
                        episode_random_r=0
                    else:
                        evaluation_random.append(episode_random_r)
                        np.save(f"results/{file_name}_{stamp}_{args.is_oiac}_{args.seed}_{args.target_type}_{train_pos}_training_random", evaluation_random)
                ...
                if args.target_type=='moving':
                    ...
                    if t% args.save_freq==0:                        
                        if args.save_freq>1:
                            ...
                            evaluation_train.append(episode_reward)
                            np.save(f"results/{file_name}_{stamp}_{args.is_oiac}_{stamp}_{args.seed}_{args.target_type}_training", evaluation_train)
                    print("---------------------------------------")
                    ...
                    if done:
                        ...
                        np.savetxt(f"tmp_sac/move_sac/EXP[1.414 0 1.414]/{hit_tar}+{file_name}+{args.is_oiac}+{args.seed}+{t}+{train_pos}",action)                    
                        evaluation_hit.append(episode_timesteps) 
                        np.save(f"results/{file_name}_{args.is_oiac}_{args.seed}_{args.target_type}_hits", evaluation_hit)   
                        episode_num += 1
                        hit_tar+=1
