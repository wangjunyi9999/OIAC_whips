...
class Actor_Gaussian(nn.Module):
    def __init__(self, args):
        ...
        self.max_action = torch.FloatTensor(args.max_action)
        self.min_action= torch.FloatTensor(args.min_action)
        self.fc1 = nn.Linear(args.state_dim, args.hidden_width)
        self.fc2 = nn.Linear(args.hidden_width, args.hidden_width)
    ...
    def forward(self, s):
        s = self.activate_func(self.fc1(s))
        s = self.activate_func(self.fc2(s))
        mean = 0.5 * (self.max_action-self.min_action) * torch.tanh(self.mean_layer(s))  # [-1,1]->[-max_action,max_action]
        return mean
