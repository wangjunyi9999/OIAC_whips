...
class TD3(object):
	...
	def train(self, replay_buffer, batch_size=256):
		...
		# for debug
		# print("action",action)
		# print("torch act:", torch.randn_like(action))
		# print(self.noise_clip)
		# print(self.min_action, "max",self.max_action)
	
		with torch.no_grad():
			# Select action according to policy and add clipped noise 
			noise = (
				torch.randn_like(action) * self.policy_noise
			).clamp(-1*self.noise_clip, self.noise_clip)
			
			next_action = (
				self.actor_target(next_state) + noise
			).clamp(self.min_action, self.max_action)
			#print("train noise:",noise, "next_act:", next_action)
