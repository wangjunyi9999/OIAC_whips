...
if __name__=="__main__":
    ...
    parser.add_argument("--eval_steps", default=10, type=int)       # How many (time steps) we evaluate  10
    ...
    # Initialize policy
        if args.policy == "TD3":
            ...
            #       policy noise 0.2
            #       noise clip 0.5
            kwargs["policy_noise"] = args.policy_noise * max_action / 2
            kwargs["noise_clip"] = args.noise_clip * max_action / 2
        ...
        for t in range(int(args.max_timesteps)):
            ...
            if t < args.start_timesteps:
                ...
                print("gen action:", action, "reward:",reward)
                model.reset()
                next_state= model.scale_observation(next_state)
                action= model.scale_action(action)
                ...
                action = (policy.select_action(np.array(state))
                    ...
                    size=action_dim)).clip(-1,1) #(min_action, max_action)
                print("policy select act:",policy.select_action(np.array(state)))
                print("noise:",np.random.normal(0, (max_action - min_action) * args.expl_noise / 2))
                print("before act:",action)
                action= model.recover_action(action)
                ...
                done_bool = float(done) if next_state <= 0.05 else 0
                replay_buffer.add(state, action, next_state, reward, done_bool)
                state = next_state
                episode_reward+=reward[0]
