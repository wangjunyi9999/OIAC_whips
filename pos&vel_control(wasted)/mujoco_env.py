...
from gym.spaces.incremental import Incremental
...
class BaseMujocoEnv(gym.Env):
    ...
    def __init__(
        ...
        """
        change 1:
        self define reacher_v5 action space: two types, 
        1: consequent
        2: discret (incremental float)
        
        Action space:[0:6]
        ini_pos_1, end_pos_1, t_1, ini_pos_2, end_pos_2, t_2
        
        """
        if model_path=="reacher_jy.xml":
            ## type 1 action: 
            self.action_space=spaces.Box(low=np.array([-0.75*np.pi, 0.25*np.pi, -0.75*np.pi, 0.25*np.pi, 1e-2]), 
                        high=np.array([-0.25*np.pi, 0.75*np.pi, -0.25*np.pi, 0.75*np.pi ,1]), 
                        shape=(5,), dtype=np.float64)
            # type 2 action : 
            # self.action_space=spaces.Box(low=np.array([-1*np.pi, 0, -1*np.pi, 0, 1e-2]), 
            #             high=np.array([0, np.pi, -0, np.pi ,1]), 
            #             shape=(5,), dtype=np.float64)
            # #TODO
            #value= Incremental(-np.pi, np.pi, 10)
            # batch_size=4
            # self.action_space= Incremental.sample(-np.pi, np.pi, 10, batch_size)
    ...
    def reset(
        ...
        *,
        ...
        self.data.qvel[1]=0
        ob = self.reset_model()
        #print("vel:", self.data.qvel[0], self.data.qvel[1])
        self.renderer.reset()
...
class MujocoEnv(BaseMujocoEnv):
    ...
    #     """
    #     change 2:
    #     if model is reacher_v5, the shape of ctrl would be (6,) 
    #     which,is theta1, theta2, w1, w2
    #     so change it to ctrl[:2] for qpos[:2] -> theta 1 and theta 2
    #     change ctrl[-2:] for qvel[:2] -> w1 ,w2
    #     """
    #     #self.data.ctrl[:] = ctrl
    #     self.data.qpos[:2] = ctrl[0], ctrl[2]
    #     # self.data.qvel[:2] = (ctrl[0]-ctrl[1])/ctrl[2], (ctrl[3]-ctrl[4])/ctrl[5]
